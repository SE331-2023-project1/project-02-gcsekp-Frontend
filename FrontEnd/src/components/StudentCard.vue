<template>
  <RouterLink :to="{ name: 'student-detail', params: { id: student.id } }">
    <div v-if="student" class="StudentDetail-class">
      <div
        class="StudentDetail-class border-2 w20-h20 rounded-lg shadow-lg mt-5 mb-5 w-250px bg-white pd-10 py-8 text-amber-500 text-base font-mono hover:bg-amber-200 cursor-pointer flex flex-col justify-center items-center"
      >
        <span class="font-bold text-xl">
          {{ `${student.FirstName.toUpperCase()} ${student.LastName.toUpperCase()}` }}</span
        >
